...
- **refresh-camera-data.yml (daily cron) → fetch-camera-data.ts + build-impact-stats.ts** — Overpass serves CI egress, so the daily refresh runs on GitHub Actions and commits camera-data.json + camera-counts.json + impact-stats.json when the SC count changes
