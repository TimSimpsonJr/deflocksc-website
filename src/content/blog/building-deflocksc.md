...
A site about surveillance shouldn't be collecting data on the people using it. If you share your location, that information never leaves your browser. If you type an address to find your reps, it goes to the US Census Bureau's geocoding service to figure out your district. We route that lookup through the site but don't store it, and the district matching itself happens on your device. One address does leave your browser by a different route: when you post a public event, we relay the address you type to OpenStreetMap so we can suggest matches, and we don't keep it. That address is about to be published on the calendar anyway, so the lookup gives away nothing the listing won't. The only thing we track is the number of clicks the "Take Action" and "Email Your Rep" buttons get, so we know how the site is actually getting used.
